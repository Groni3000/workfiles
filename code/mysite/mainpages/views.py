...
def result_of_search_page(request):
    ...
    (В идеале, в этом месте следует прервать функцию и последующий код сделать в другой функции
    ради реюзабельности. 
    То есть, check_criterion - проверяет критерий; 
    Новая функция - выбирается НУЖНАЯ таблица ... ну, то есть, пояснение ниже)
    выбирается НУЖНАЯ таблица из БД, формирование ответа на поисковый запрос;
    ------------------------------------------------------------------------------------------------------
    data_for_pagination(result_of_search, request, 16)
    ...
    ------------------------------------------------------------------------------------------------------
    data_for_presentation_on_page(search_query, page, table, (0,1))
